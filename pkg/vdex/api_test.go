...
func buildTestVDEX(t *testing.T) []byte {
	...
	pu32(dex, 0x80, 0); pu32(dex, 0x84, 0); pu32(dex, 0x88, 0)
	// field_ids[0]: class=0, type=0, name=1
	pu16(dex, 0x8C, 0); pu16(dex, 0x8E, 0); pu32(dex, 0x90, 1)
	// method_ids[0]: class=0, proto=0, name=1
	pu16(dex, 0x94, 0); pu16(dex, 0x96, 0); pu32(dex, 0x98, 1)
	...
	pu32(dex, 0xA8, 0); pu32(dex, 0xAC, 0xFFFFFFFF)
	pu32(dex, 0xB0, 0); pu32(dex, 0xB4, 0); pu32(dex, 0xB8, 0)
	// map_list @ 0xBC
	pu32(dex, 0xBC, 1)        // 1 map item
	pu16(dex, 0xC0, 0x1000)   // TYPE_MAP_LIST
	pu16(dex, 0xC2, 0)
	pu32(dex, 0xC4, 1); pu32(dex, 0xC8, 0xBC)
	// string data @ 0xCC
	dex[0xCC] = 7; copy(dex[0xCD:], "LHello;\x00")
	dex[0xD5] = 4; copy(dex[0xD6:], "Test\x00")
	...
	pu32(buf, 12, 0); pu32(buf, 16, checksumOff); pu32(buf, 20, checksumSize)
	// section[1] kDexFileSection
	pu32(buf, 24, 1); pu32(buf, 28, dexOff); pu32(buf, 32, uint32(dexSize))
	// section[2] kVerifierDepsSection (empty)
	pu32(buf, 36, 2); pu32(buf, 40, dexOff+uint32(dexSize)); pu32(buf, 44, 0)
	// section[3] kTypeLookupTableSection (empty)
	pu32(buf, 48, 3); pu32(buf, 52, dexOff+uint32(dexSize)); pu32(buf, 56, 0)
...
func TestWithDexPreview_AcceptsOption(t *testing.T) {
	data := buildTestVDEX(t)
	// Just verify option doesn't panic or error
	r, err := vdex.ParseBytes(data, vdex.WithDexPreview(10))
	require.NoError(t, err)
	require.NotNil(t, r)
}

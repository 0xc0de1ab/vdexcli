...
- 6개 출력 포맷 (text/json/jsonl/summary/sections/coverage)
- 구조화된 진단 시스템 (33개 진단 코드)
- 98개 테스트 (e2e + 단위 + 통합)
...
| modifier/presenter/dex 패키지 테스트 없음 | parser(88.9%)와 extractor만 단위 테스트 보유 |
...
- `--format summary` 출력을 CI 게이트로 활용하는 예시
- pre-commit hook으로 VDEX 수정 시 자동 검증

- 복잡도: **L**
- 선행조건: 없음

### 4.4 flagsbinder 마이그레이션
...
```
v0.3: Phase 1 (안정성)
  1.1 테스트 커버리지 → 1.2 modifier 분할 → 1.3 인터페이스

v0.4: Phase 2 (수정 확장)
  2.4 DM 파싱 → 2.1 섹션 재배치 → 2.3 checksum → 2.2 type-lookup

v0.5: Phase 3 (멀티 버전)
  3.1 v021~v026 → 3.2 v028+

v1.0: Phase 4 (생태계)
  4.1 diff → 4.2 create → 4.3 CI → 4.4 flagsbinder
```
